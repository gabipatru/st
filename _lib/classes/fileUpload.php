...
class fileUpload {
	...
	function __construct() {
		...
		$this->iMaxFileSize = 999999999;
		$this->sMimeCheck = 'mp3';
	}
	
	/*
	 * define the phisicat path to which the files will be uploaded
	 */
	...
	/*
	 * Sets the filed name as is in _FILES
	 */
	...
	*/
	 ...
	 */
	public function setFileExtension($sExt) {
		$this->sFileExtension = $sExt;
	}
	
	/*
	 * This function sets the maximum file size, in bytes 
	 */
	...
	/*
	 * Set only the jpg, png and gif as allowe
	 */
	public function addImageTypes($sType) {
	...
	/*
	 * Returns the phisical upload path
	 */
	...
	/*
	 * Get the field name as is in _FILES
	 */
	...
	public function Upload() {
		...
		$this->mimeType = $this->getMimeType($this->sMimeCheck);
		if (!$this->checkMimeType($this->mimeType)) {
			return false;
		}
		
		// check the file size
		if (filesize($_FILES[$this->getFieldName()]['tmp_name']) > $this->iMaxFileSize) {
			return false;
		}
		...
		if(!move_uploaded_file($_FILES[$this->getFieldName()]['tmp_name'], $this->getUploadPath().'/'.$this->getFileName().'.'.$sExt)) {
			return false;
		}
